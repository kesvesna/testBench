...
long long testSizeBuffer = 1000; //2147483647; // 2Gb // *64 byte
alignData * buffer = (alignData *) malloc(testSizeBuffer*64);
...
//======================================================================
		...
		for ( long long i = 0; i < operations; ++i )
		{
			...
			{
				functionCASAsm (&buffer->structInt, &buffer[j].structInt, &buffer[j].structInt, &buffer[j].structInt);
				buffer++;
			}
		}
		endRdtsc = rdtsc();
...
//======================================================================
		...
		for ( long long i = 0; i < operations; ++i)
		{
			for ( long long j = 0; j < testSizeBuffer; ++j )
			{
				functionFAAAsm (&buffer[j].structInt,&buffer->structInt);
				buffer++;
			}
		}
		endRdtsc = rdtsc();
...
int main ()
{
	...
	for (int ix = 0; ix < testSizeBuffer; ix++)
	{
		...
		buffer[ix].structInt = 'a'; // для отладки, быстрое заполнение буфера, закоментить после отладки
	}
	fout.open("testbenchAlignedIntStructBuffer.m");
	time_t myTime;
	myTime = time(NULL);
	...
	cout << "Buffer size: " << testSizeBuffer*64 << " bytes" << endl;
	...
	fout << "grid on" << endl;
	fout << "titleString = sprintf('bandwidth test with Aligned Int Data , testRuns = %d, buffer size = %d bytes', testRuns, bufferSize)" << endl;
