...
long long testSizeBuffer = 8000; // 2Gb = 33554432 testSizeBuffer, 2Mb = 32768 testSizeBuffer, 16Kb = 256
...
inline void invalidateCash(volatile int *ptr)
{
...
inline void modifideCash (volatile int *ptr)
{
	for ( int i = 0; i < testSizeBuffer; ++i)
	{
		*ptr = 5;
...
inline void exclusiveCash (volatile int *ptr)
{
	int tmpValue;
	for ( int i = 0; i < testSizeBuffer; ++i)
	{
...
//======================================================================
		...
		for ( long long i = 0; i < iterations; ++i )
		{
			...
			if ( !dataSizeIsStored )
			{
				fout << "arrDataSize = [ arrDataSize " << bufferSize * sizeof(int) << " ]; " << endl;
...
//======================================================================
			...
			//asm volatile ("":::"memory");
			//exclusiveCash(buffer);
			//modifideCash(ptrGlobalVariable);
			//pthread_barrier_wait(&open_barrier);
			//warmUp();
			startRdtsc = rdtsc();
			for ( long long j = 0; j < bufferSize; ++j )
			{
				functionFAAAsm (&buffer, &buffer);
			}
			endRdtsc = rdtsc();
			faaTimeVector.push_back((double)(endRdtsc-startRdtsc)/iterations);
		}
		cout << "test for data size " << bufferSize * sizeof(int) << " bytes finished" << endl;
		if ( bufferSize <= 3839 )
		{
		bufferSize += 256;
		}
		else if ( bufferSize > 3839 && bufferSize <= 4200 )
		{ bufferSize += 64; }
		else
		{
			bufferSize += 512;
...
int main ()
{
	...
	//int i = 0;
	//while ( i < testSizeBuffer)
	//{
	//	cout << " ; " << *buffer << endl;
	//	buffer++;
	//	i++;
	//}
	fout.open("latencyTestForICashStateWithIntJustNowNoBarrier.m");
	time_t myTime;
	myTime = time(NULL);
	...
	fout << "grid on" << endl;
	fout << "titleString = sprintf('latency test for Invalidate Cash with Int and no barrier, testRuns = %d, iterations = %d, size of buffer = %d ', testRuns, iterations, bufferSize);" << endl;
