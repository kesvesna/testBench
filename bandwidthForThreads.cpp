...
long long warmUpOperations = 1000000;
...
int * buffer = (int *) malloc(testSizeBuffer);
...
//======================================================================
		...
		startRdtsc = rdtsc();
			...
			{
				functionCASAsm (ptrGlobalVariable,&globalVariable,&globalVariable,&globalVariable);
			}
		endRdtsc = rdtsc();
...
//======================================================================
		...
		startRdtsc = rdtsc();
			for ( long long i = 0; i < operations; ++i)
			{
				functionFAAAsm (&globalVariable,ptrGlobalVariable);
			}
		endRdtsc = rdtsc();
...
int main ()
{
	...
	for (int ix = 0; ix < testSizeBuffer; ix++)
	{
		...
		buffer[ix] = 3; // для отладки, быстрое заполнение буфера, закомментить после отладки
	}
	fout.open("resultWithDataBuffer.m");
	time_t myTime;
	myTime = time(NULL);
	...
	fout << "grid on" << endl;
	fout << "legend('Cas','Faa','North')" << endl;
