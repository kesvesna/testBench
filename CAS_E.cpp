...
//======================================================================
//======================================================================
void * prep_E (void * arg) // prepair for E state 
{
	auto get_time = std::chrono::steady_clock::now;
	decltype(get_time()) start, end;
	unsigned long long dataSize = startDataSize;
	...
	while (dataSize <= testSizeBuffer)
		{
			for (auto i = 0; i < nruns; i++) 
				{
					while (flagMState.load() == false) {} // wait ready for state E
					start = get_time();
					for (auto j = 0; j < dataSize; j++)
						{
							buffer[j].compare_exchange_weak(exp, des); // CAS
...
int main(int argc, const char *argv[])
{
	...
	//==================================================================
	fout.open("CAS_E_cpu2_cpu3_cpu1.m"); // open stream for writing in file
	// Architecture =====================================================
	...
	pthread_create(&threads[i],&attr, exclusive_local, NULL);
	...
	for (int i = 0; i <= 1; i++) 
		{	  
			...
			if ( i == 0 )
				{
					pthread_create(&threads[i],&attr, prep_E, NULL); // thread on cpu2
				}
			if ( i == 1 )
				{
	...
	fout << "titleString = sprintf('CAS, E, CPU2 (local), CPU3, CPU1');" << endl;
	...
	fout << "titleString = sprintf('CAS, M, CPU2 (local), CPU3, CPU1');" << endl;
	fout << "title (titleString);" << endl;
