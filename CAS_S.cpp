...
//======================================================================
//======================================================================
void * shared_local (void * arg) // prepair for state 
{
			...
			for (auto i = 0; i < nruns; i++) 
				{
					...
					clflush(buffer,dataSize);              // local invalid
					for (auto j = 0; j < dataSize; j++)
						{
							buffer[j].load();             // local exclusive
						}
					flagCPU1.store(true);
...
//======================================================================
//CPU2 shared
void * shared_local2 (void * arg) // prepair for state 
{
	auto get_time = std::chrono::steady_clock::now;
    decltype(get_time()) start, end;
    unsigned long long dataSize = startDataSize;
			...
			for (auto i = 0; i < nruns; i++) 
				{
					...
					for (auto j = 0; j < dataSize; j++)
						{
							buffer[j].load(); // cpu1 and cpu2 shared
						}
					flagCPU3.store(true);
					flagCPU1.store(false);
				}
...
int main(int argc, const char *argv[])
{
	...
	//==================================================================
	fout.open("CAS_S_cpu2_cpu3_cpu1.m"); // open stream for writing in file
	// Architecture =====================================================
	...
	//==================================================================
	//  two threads=====================================================
	// shared measuring for CPU2 local core
	flagCPU1.store(false);
	...
	for (int i = 1; i <= 2; i++) 
		{	  
			...
			if ( i == 2 )
				{
					pthread_create(&threads[i],&attr, shared_local, NULL);
				}
		}
		for (int i = 1; i <= 2; i++) 
			{
	...
	//==================================================================
	//  two threads=====================================================
	// shared measuring for cpu3 near core
	flagCPU1.store(false);
	flagCPU2.store(true);
	flagCPU3.store(false);
	clflush(buffer,testSizeBuffer);
	for (int i = 1; i <= 3; i++) 
		{	  
			...
			if ( i == 1 )
				{
					pthread_create(&threads[i],&attr, meas_S_CAS2, NULL);
				}
			if ( i == 2 )
				{
					pthread_create(&threads[i],&attr, shared_local2, NULL);
				}
			if ( i == 3 )
				{
					pthread_create(&threads[i],&attr, meas_S_CAS3, NULL);
				}
		}
	for (int i = 1; i <= 3; i++) 
		{
	...
	//==================================================================
	//  two threads=====================================================
	// shared measuring for cpu1 remote core
	flagCPU1.store(false);
	flagCPU2.store(true);
	flagCPU3.store(false);
	clflush(buffer,testSizeBuffer);
	...
	fout << "titleString = sprintf('CAS, S, CPU2 (local), CPU3, CPU1');" << endl;
